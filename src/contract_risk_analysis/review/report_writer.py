...
2. 退让范围：从目标到可接受底线的阶梯
   ...
   - **退让数字纪律（强制执行）**：退让阶梯中如果出现百分比、天数或金额目标值，这些目标值只能来自：（a）本合同原文中的已出现数字（如”从80%降低”），（b）Dossier 定量锚点中的换算公式或金额。**严禁**写出本合同和 Dossier 均未出现过的固定目标数字（如”20%””30%””10%”等）。如果没有可追溯来源，退让阶梯必须写成方向+保护条件的形式（如”显著降低前置付款比例，不低于质保金担保倍数”，”将质保金提交节点提前至买方资金敞口被担保之后”），不得在此处自行补数字。
   - 如果第六章对某付款/风险条款设置了“在缺少某项保护条件时不得超过X%/不得接受某节点”的签署底线，则本节出现比该底线更宽的退让阶梯时，必须同步写明使该退让成立的保护前提；不得把更宽的退让空间写成无条件底线。
