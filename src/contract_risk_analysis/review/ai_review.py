...
def _build_chip_instruction(review_party: str, party_label: str) -> str:
    ...
    if review_party == "buyer":
        examples = (
            ...
            "例：对方坚持保留的付款、验收、风险转移或违约责任安排——"
            "己方只能在获得对应保护条件或其他关键让步时，才做有限退让。\n"
            "③ **响应筹码** — 对己方有利或中性，但对方有生存级恐惧、必然主动要求修改的条款。"
            "策略原则：绝不主动提出修改；等对方提出时，将'同意修改'作为交换条件。"
            "例：合同中保留了对己方有利的责任范围、争议解决或解除安排——"
            "可将'同意局部调整'作为交换条件，换取对方在核心风险条款上的让步。\n"
        )
    else:
        examples = (
            ...
            "② **交换筹码** — 对己方不利但对方极度想保留的条款。策略原则：主动出牌——"
            "用'愿意在此条款上做有控制的让步'换取对方在底线条款上的妥协。"
            "例：对方坚持保留的付款、验收、风险转移或责任安排——"
            "己方只能在换得对应保护条件或其他关键让步时，才做有限退让。\n"
            "③ **响应筹码** — 对己方有利或中性，但对方有生存级恐惧、必然主动要求修改的条款。"
            "策略原则：绝不主动提出修改；等对方提出时，将'同意修改'作为交换条件。"
            "例：对方希望削弱己方已获得的程序或责任保护——"
            "可将'同意局部调整'作为交换条件，换取对方在核心风险条款上的让步。\n"
        )
    return (
...
def _free_review_prompt(
    ...
    return (
        ...
        "7. **数字纪律**：所有百分比、金额、天数都必须能直接追溯到合同原文。"
        "只有当合同原文明确出现合同总价、价税合计或其他可作为总额基准的金额时，"
        "才允许把百分比换算成金额；若总价未明确，只能保留百分比/天数，"
        ...
        "否则担保形同虚设。若同时存在[高额预付款]与[担保提交滞后/质保机制不足]，"
        "应优先识别为一个主风险（付款担保结构失衡/资金结构倒挂），再将质保不足写成补充性子风险，"
        "避免把同一资金暴露逻辑拆成两个平级、重复展开的大风险。\n"
        ...
        "对‘视为交付/签字即交付/逾期未提异议视为合格’类条款，必须同时判断两面："
        "其一，是否提前触发交付完成或风险转移解释；"
        "其二，合同中是否仍保留质量责任、拒收、退换货或终验保护。"
        "出现[签字即视为交付]、[逾期未提出异议视为合格]等表述时，若保护不足必须标注风险。\n\n"
        "## 输出字段说明\n"
        ...
        "- 不要输出总体风险结论（由后续环节处理）\n"
        "- 如果文本无法支持某项，不要臆造\n"
        "- 数字只能来自合同原文；不得为了增强说服力补写无来源金额、比例或天数\n"
        "- 若合同未明确总价，不得把比例换算为金额；请直接写“合同总价未明确，暂不进行金额测算”\n"
